...
private:
	...
	void deleteLastChar() {
		ofstream text1("../input.txt", std::ios::app);
		ifstream text_1("../input.txt", std::ios::app);
		std::string t = text.str();
		std::string newT = "";
		for (int i = 0; i < t.length() - 1; i++) {
			newT += t[i];
		...
		getline(text_1, s);
		s.pop_back();
		std::fstream("../input.txt", std::fstream::out);
		text1 << s;
	}
